...
parser.add_argument(
    "input", help="The input JSON for comparison.", type=argparse.FileType("r")
)
parser.add_argument(
    "--reference",
    "-r",
...
if type(REFERENCE) != list:
    print(
        "The reference JSON should be a list. Please check if you pick a wrong reference file."
    )
...
def compareIntegerQueueEntry(i: dict, r: dict) -> bool:
    ...
    for n, t in types.items():
        if not n in r:
            print(
                f"The reference integer queue entry has wrong format. Missing term: {n}. Raw: {r}."
            )
    ...
    for exact_entry in ["PC", "OpCode", "DestRegister", "OpAIsReady", "OpBIsReady"]:
        if not exact_entry in i:
            print(
                f"[{RED}Error{RESET}][IntegerQueue] Missing property for the integer queue entry (PC={i['PC']}): {exact_entry}."
            )
            return False
        ...
        if type(i[exact_entry]) != type(r[exact_entry]):
            print(
                f"[{RED}Error{RESET}][IntegerQueue] Property type mismatched for the integer queue entry (PC={i['PC']}): {exact_entry} should be {type(r[exact_entry])}"
            )
            return False
        ...
        if i[exact_entry] != r[exact_entry]:
            print(
                f"[{RED}Error{RESET}][IntegerQueue] Mismatched property in the integer queue entry (PC={i['PC']}): {exact_entry}."
            )
            if exact_entry == "PC":
                print(
                    f"[{RED}Error{RESET}][IntegerQueue] A PC mismatching usually means you are missing entries for some instructions."
                )
    ...
    for op in ["A", "B"]:
        if i[f"Op{op}IsReady"]:
            ...
            if not f"Op{op}Value" in i:
                print(
                    f"[{RED}Error{RESET}][IntegerQueue] Missing property for the integer queue entry (PC={i['PC']}): Op{op}Value."
                )
                return False
            if i[f"Op{op}Value"] != r[f"Op{op}Value"]:
                print(
                    f"[{RED}Error{RESET}][IntegerQueue] Mismatched property in the integer queue entry (PC={i['PC']}): Op{op}Value."
                )
                return False
        else:
...
def compareIntegerQueue(i: list[dict], r: list[dict]) -> bool:
    ...
    for el in r:
        if not "PC" in el:
            print(
                "All integer queue entry in reference output should have property 'PC'. "
            )
            print("Please check if the reference file is correct.")
            exit(2)
    ...
    for el in i:
        if not "PC" in el:
            print(
                f"[{RED}Error{RESET}][IntegerQueue] All integer queue entry should have property 'PC'. "
            )
            return False
    ...
    if len(i) != len(r):
        print(
            f"[{RED}Error{RESET}][IntegerQueue] The number of elements in the integer queue is different from reference."
        )
        print(
            f"[{RED}Error{RESET}][IntegerQueue] This usually means you are missing some instructions, or you have more instructions retired later."
        )
        return False
...
def compareActiveListEntry(i: dict, r: dict) -> bool:
    ...
    for idx, t in types.items():
        if not idx in i:
            print(
                f"[{RED}Error{RESET}][ActiveList] Missing property in active list entry: {idx}"
            )
            return False
        ...
        if type(i[idx]) != t:
            print(
                f"[{RED}Error{RESET}][ActiveList] Mismatched type for a property in an active list entry: {idx}"
            )
            return False
        ...
        if i[idx] != r[idx]:
            print(
                f"[{RED}Error{RESET}][ActiveList] Mismatched value of a property in an active list entry: {idx}"
            )
            return False
...
def compareActiveList(i: list[dict], r: list[dict]) -> bool:
    ...
    for idx in range(len(i)):
        if compareActiveListEntry(i[idx], r[idx]) == False:
            print(
                f"[{RED}Error{RESET}][ActiveList] Active list entry at index {idx} is different."
            )
            return False
...
def compareCycleData(i: dict, r: dict) -> bool:
    ...
    for n, t in types.items():
        ...
        if type(r[n]) != t:
            print(
                f"Wrong type of the property in the cycle data in the reference input: {n}. The expected type is {t}"
            )
            print("Please check if the reference file is correct.")
            exit(2)
    ...
    for n, t in types.items():
        if not n in i:
            print(
                f"[{RED}Error{RESET}][CycleData] Missing property in the cycle data in the input: {n}"
            )
            return False

        if type(i[n]) != t:
            print(
                f"[{RED}Error{RESET}][CycleData] Wrong type of the property in the cycle data in the input: {n}. The expected type is {t}"
            )
            return False
    ...
    for directed_check in [
        ...
        if i[directed_check] != r[directed_check]:
            print(f"i : {i[directed_check]}")
            print(f"r : {r[directed_check]}")
            print(
                f"[{RED}Error{RESET}][CycleData] Property '{directed_check}' mismatched."
            )
            return False
    ...
    if r["Exception"] == True:
        ...
        if type(i["ExceptionPC"]) != int:
            print(
                f"[{RED}Error{RESET}][CycleData] Type mismatched: Property ExceptionPC should be an integer."
            )
            return False
